...
!> @brief   Initialise IO_Dev data to the product of its X, Y and Z coordinates
...
subroutine io_dev_init_code( nlayers,                    &
  ...
  integer(kind=i_def), intent(in) :: ndf_x, ndf_chi, ndf_pid
  integer(kind=i_def), intent(in) :: undf_x, undf_chi, undf_pid
