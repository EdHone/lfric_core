    ...
    module scale_by_cos_lat_kernel_mod
    ...
    type, public, extends(kernel_type) :: scale_by_cos_lat_kernel_type
    private
    ...
    contains
    procedure, nopass :: scale_by_cos_lat_code
    end type scale_by_cos_lat_kernel_type

    public scale_by_cos_lat_code
    contains
    ...
    subroutine scale_by_cos_lat_code(nlayers,   &
                                     field_out, &
                                     field_in,  &
                                     latitude,  &
                                     ndf_f,     &
    ...
    !Value of cosine
    real(kind=r_def) :: cos_lat(ndf_2d)


    ! apply scaling
    do df = 1, ndf_2d
      cos_lat(df)=COS(latitude(map_2d(df)))
    end do

    do k = 0, nlayers-1
      do df = 1, ndf_f
        field_out(map_f(df) + k) = field_in(map_f(df) + k ) * cos_lat(df)
      end do
    end do

    end subroutine scale_by_cos_lat_code

    end module scale_by_cos_lat_kernel_mod
