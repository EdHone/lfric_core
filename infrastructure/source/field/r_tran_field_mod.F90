...
module r_tran_field_mod
  ...
  use constants_mod,      only: r_def, r_double, i_def, i_halo_index, l_def, &
                                str_def, r_tran, r_tran_real_type
...
!______r_tran_field_type_____________________________________________________

  !> Algorithm layer representation of an r_tran field.
  !>
  ...
  !> alone.
  !>
  type, extends(field_parent_type), public :: r_tran_field_type
    private

    !> The r_tran values of the field
    real(kind=r_tran), allocatable :: data( : )
    !> Enable field to point to bespoke data provided by application
    !> instead of having allocated data
    real(kind=r_tran), pointer :: override_data( : )
  ...
  contains
    ...
    ! Routine to copy data from an r_tran_field_type into a field_type
    procedure, public :: copy_to_rdef

    ! Routine to copy data from a field_type into an r_tran_field_type
    procedure, public :: copy_from_rdef
  ...
  end type r_tran_field_type

!______r_tran_field_pointer_type_______________________________________________________

!> a class to hold a pointer to an r_tran field in an object that is a child
!> of the pure abstract field class

  type, extends(pure_abstract_field_type), public :: r_tran_field_pointer_type
    private
    !> A pointer to an r_tran field
    type(r_tran_field_type), pointer, public :: field_ptr
  contains
    !> Initialiser for a field. May only be called once.
    procedure, public :: r_tran_field_pointer_initialiser
    !> Finaliser for a field pointer object
    final :: r_tran_field_pointer_destructor
  end type r_tran_field_pointer_type
...
!______field_proxy_type_______________________________________________________
  ...
  !>
  type, extends(field_parent_proxy_type), public :: r_tran_field_proxy_type

    private

    !> Allocatable array of type r_tran which holds the values of the field
    real(kind=r_tran), public, pointer :: data( : ) => null()
  ...
  end type r_tran_field_proxy_type

!______end of type declarations_______________________________________________

contains

!______r_tran_field_type_procedures__________________________________________
  ...
  !> field_type
  type(r_tran_field_proxy_type) function get_proxy(self)
    implicit none
    class(r_tran_field_type), target, intent(in)  :: self
  ...
  !> Initialise an r_tran field pointer
  !>
  !> @param [in] field_ptr A pointer to the r_tran field that is to be
  !>                       stored as a reference
  subroutine r_tran_field_pointer_initialiser(self, field_ptr)
    implicit none
    class(r_tran_field_pointer_type) :: self
    type(r_tran_field_type), pointer :: field_ptr
    self%field_ptr => field_ptr
  end subroutine r_tran_field_pointer_initialiser

  ! Finaliser for an r_tran field pointer
  !
  ...
  subroutine r_tran_field_pointer_destructor(self)
    implicit none
    type(r_tran_field_pointer_type), intent(inout) :: self
  end subroutine r_tran_field_pointer_destructor
  ...
  subroutine copy_field(self, dest, name)
    ...
    implicit none
    class(r_tran_field_type), target, intent(in)  :: self
    class(r_tran_field_type), target, intent(out) :: dest
    character(*), optional, intent(in)              :: name
  ...
  !> Routine to copy data from an r_tran_field_type into a field_type
  !>
  !> @param[out] dest   field_type object from which the copy will be made
  subroutine copy_from_rdef(self, dest)
    implicit none
    class(r_tran_field_type), target, intent(inout)  :: self
    class(field_type), target, intent(in) :: dest

    type(field_proxy_type) :: fp

    fp = dest%get_proxy()
    if (.not. self%is_initialised())then
      call self%initialise( vector_space = dest%get_function_space(), &
                            name = dest%get_name()//"_mp" )
    end if
    self%data(:)=real(fp%data(:),kind=r_tran)
    call dest%copy_field_parent(self)

  end subroutine copy_from_rdef

  !> Routine to copy data from a field_type into an r_tran_field_type
  !>
  !> @param[out] dest   field_type object to which the copy will be made
  subroutine copy_to_rdef(self, dest)
    implicit none
    class(r_tran_field_type), target, intent(inout)  :: self
    class(field_type), target, intent(inout) :: dest

    type(field_proxy_type) :: fp

    fp = dest%get_proxy()
    fp%data(:)=real(self%data(:),kind=r_def)
    call self%copy_field_parent(dest)

  end subroutine copy_to_rdef
  ...
  subroutine field_type_assign(dest, source)
    ...
    use log_mod,         only : log_event, &
                                ...
                                LOG_LEVEL_INFO
    implicit none
    class(r_tran_field_type), intent(in)  :: source
    class(r_tran_field_type), intent(out) :: dest
  ...
  function is_initialised(self) result(initialised)
    implicit none
    class(r_tran_field_type), intent(in) :: self
  ...
  subroutine field_destructor_scalar(self)
    ...
    type(r_tran_field_type), intent(inout)    :: self
  ...
  subroutine log_minmax( self, log_level, label )
    ...
    type(function_space_type),               pointer :: function_space => null()
    integer(i_def)                                   :: i
    real(r_tran)                                     :: l_min, l_max
    real(r_tran)                                     :: answer_min, answer_max
  ...
  subroutine field_minmax( self, fmin, fmax )
    ...
    class( r_tran_field_type ), target, intent(in) :: self
    type(function_space_type),      pointer :: function_space => null()
    real(r_tran) , intent(out)              :: fmin, fmax
    integer(i_def)                          :: undf
    ...
    fmin1 = scalar_type( real(minval( self%data(1:undf) ), kind=r_def) )
    fmax1 = scalar_type( real(maxval( self%data(1:undf) ), kind=r_def) )

    fmin = real(fmin1%get_min(), kind=r_tran )
    fmax = real(fmax1%get_max(), kind=r_tran )
  ...
  subroutine write_field(this, field_name)
    ...
    class(r_tran_field_type), intent(in)     :: this
  ...
  subroutine read_field( self, field_name)
    ...
    type( r_tran_field_proxy_type )                        :: tmp_proxy
  ...
  subroutine halo_exchange( self, depth )
    ...
    class( r_tran_field_proxy_type ), target, intent(inout) :: self
  ...
  function get_sum(self) result (answer)
    ...
    class(r_tran_field_proxy_type), intent(in) :: self
