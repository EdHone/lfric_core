...
!-------------------------------------------------------------------------------
!>  @brief    Performs an XIOS domain initialisation
!!
!!  @details  Performs domain init and returns
...
subroutine xios_domain_init(xios_ctx, mpi_comm, dtime, mesh_id, chi, &
 ...
 ! Do Node domain setup
  ...
  call xios_set_axis_attr("vert_axis_node", n_glo=size(fractional_levels_nodes), value=fractional_levels_nodes)

  ! Clean up things used ready to reuse for face domain setup
...
!------------------------------------------------------------------------------- 
subroutine nodal_write_field(nodal_coordinates, level, nodal_output, fspace_dimension, output_unit, fname)
...
subroutine xios_write_field_node(xios_field_name, field_proxy)
  ...
  do i=0, axis_size-1
    send_field(i*(domain_size)+1:(i*(domain_size)) + domain_size) = field_proxy%data(i+1:undf:axis_size)
  end do

  ! Reshape into 2D horizontal + vertical levels for output
  call xios_send_field(xios_field_name, reshape (send_field, (/domain_size, axis_size/) ))
