...
class TestFortranDependency:
    ...
    def test_get_compile_prerequisites_sub_tree(
        self,
        example_db: FortranDependencies
    ):
        assert sorted(example_db.get_compile_prerequisites('bar')) == \
            ['qux']
    ...
    def test_get_modules(self, example_db: FortranDependencies):
        ...
        assert [('noonoo', Path('foo.f90')), ('bar', Path('bar.f90')),
                ('baz', Path('baz.f90')), ('qux', Path('qux.f90')),
                ('wilma', Path('wilma.f90'))] == list(modules)

    def test_get_all_compile_dependencies(self,
                                          example_db: FortranDependencies):
        """
        Ensure all file dependencies are correctly returned.
        """
        assert sorted(example_db.get_compile_dependencies()) == \
               [('bar', Path('bar.f90'), 'module',
                 'qux', Path('qux.f90'), 'module'),
                ('foo', Path('foo.f90'), 'program',
                 'bar', Path('bar.f90'), 'module'),
                ('foo', Path('foo.f90'), 'program',
                 'baz', Path('baz.f90'), 'module'),
                ('foo', Path('foo.f90'), 'program',
                 'noonoo', Path('foo.f90'), 'module'),
                ('fred', Path('fred.f90'), 'program',
                 'wilma', Path('wilma.f90'), 'module'),
                ('noonoo', Path('foo.f90'), 'module',
                 'wilma', Path('wilma.f90'), 'module')]

        assert [('bar', Path('bar.f90'),
                 'foo', Path('foo.f90'))] \
            == list(example_db.get_link_dependencies('bar'))
        assert [('baz', Path('baz.f90'),
                 'foo', Path('foo.f90'))] \
            == list(example_db.get_link_dependencies('baz'))
    ...
    def test_get_some_compile_dependencies(self,
                                           example_db: FortranDependencies):
        """
    ...
    def test_get_submod_compile_dependencies(self, tmp_path: Path):
        ...
        assert sorted(database.get_compile_dependencies()) == \
            [
                ('child1', Path('parent.f90'), 'submodule',
                 'parent', Path('parent.f90'), 'module'),
                ('child2', Path('child2.f90'), 'submodule',
                 'parent', Path('parent.f90'), 'module'),
                ('prog', Path('prog.f90'), 'program',
                 'parent', Path('parent.f90'), 'module')
            ]
