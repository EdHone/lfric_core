...
subroutine gp_rhs_code(nlayers, &
                       ndf, map, basis, rhs, gq, &
                       ndf_f, map_f, f_basis, field, &
                       ndf_chi, map_chi, chi_diff_basis, chi_1, chi_2, chi_3 &
                       )
  ...
  real(kind=r_def), intent(in), dimension(1,ndf_f,  ngp_h,ngp_v) :: f_basis 
  real(kind=r_def), intent(in), dimension(3,ndf_chi,ngp_h,ngp_v) :: chi_diff_basis 

  real(kind=r_def), intent(inout) :: rhs(*)
       ...
       do qp2 = 1, ngp_v
          do qp1 = 1, ngp_h
            ...
            rhs(map(df) + k) = rhs(map(df) + k) &
                             + 0.125_r_def*wgp_h(qp1)*wgp_v(qp2)*basis(1,df,qp1,qp2) &
                             * rhs_cell * dj(qp1,qp2)
          end do
       end do      
    end do
