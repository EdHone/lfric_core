...
type, extends(abstract_external_field_type), public :: coupler_exchange_2d_type
  ...
  ! Time of next timestep in seconds from start of the run
...
contains
  ...
  !> Checks if the next timestep is scheduled for a coupling operation
  ...
  subroutine set_time(self, model_clock)
    ...
    ! Store time of next timestep in seconds from start of the run
    self%coupling_time_next = &
        int( model_clock%seconds_from_steps(model_clock%get_step()+1)        &
              - model_clock%seconds_from_steps(model_clock%get_first_step()), &
              i_def)
  ...
  !> @brief   Checks if the next timestep is scheduled for a coupling
  !>          operation
  !> @return  Whether the next timestep is scheduled for coupling
  !
